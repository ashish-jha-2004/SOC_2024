...
void solve(){
    ...
    ll ans1, ans2;
    if (v3.size() <= 0 and (v1.size() <= 0 or v2.size() <= 0)){
        cout << "-1" << en;
        return;
    }
    if (v1.size() <= 0 or v2.size() <= 0) {
        cout << *min_element(all(v3)) << en;
        return;
    }
    if (v3.size() <= 0){
        cout << *min_element(all(v1))+*min_element(all(v2)) << en;
        return;
    }
    cout << min(*min_element(all(v1))+*min_element(all(v2)), *min_element(all(v3))) << en;
}
