...
template <class T> 
using ind_multiset = tree<T, null_type, less_equal<T>, rb_tree_tag, tree_order_statistics_node_update>; 
template <typename T>
void pt(T &&t)  { cout << t << "\n"; }
template<typename T>
...
void solve(){
    ...
    cin >> n >> l >> r;
    d_v(v, n);
    ind_set<pair<ll, ll>> s;
    ll ans = 0;
    for (int i{n-1}, tim{0}; i>=0; i--, tim++) {
        ll low = s.order_of_key({l-v[i], -1});
        ll high = s.order_of_key({r - v[i], INT32_MAX});
        s.insert({v[i], tim});
        ans += high - low;
