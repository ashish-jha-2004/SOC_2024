...
bool hasMoreThanOneSameElement(const std::vector<ll>& arr) {
...
ll modularMultiplication(vl &arr) {
    long long result = 1; 
    int n = arr.size();
    for (int i = 0; i < n; ++i) {
        for (int j = i + 1; j < n; ++j) {
            result = mod_mul(result, (arr[i] % arr[j]));
        }
    }
    return result;
}
...
void solve(){
    ...
    if (hasMoreThanOneSameElement(v)) {
        cout << 0 ;
        return;
    }
    cout << modularMultiplication(v);
}
